...
//System.out.println();
        ...
        do {
            ...
            dualSplit(dualList, 0, dualList.length - 1);
            end= new Date();
            rounds++;
//	System.out.println();
        }while(end.getTime()-start.getTime()<1000);
        ...
        do {
            doubleDupe = dupeList;
            dualSplit(doubleDupe, 0, doubleDupe.length - 1);
            end= new Date();
            rounds++;
        }while(end.getTime()-start.getTime()<1000);
    ...
    public static void singlePivot(int[] list,  int minIndex, int maxIndex){
        if(maxIndex-minIndex>2){
            ...
            singlePivot(list, splitIndex, maxIndex);
...
*/
    public static void dualSplit(int[] list, int minIndex, int maxIndex){
...
*/
	    ...
	    int pivot1 = list[minIndex];
	    int pivot2 = list[minIndex];
	    int lt = minIndex + 1;
	    int gt = maxIndex - 1;
	    int i = minIndex + 1;
	    while(i <= gt) {
	    ...
	    swap(list, minIndex, lt-1);
	    swap(list, gt+1, maxIndex);
	    lt--;
	    gt++;
	    dualSplit(list, minIndex, lt-1);
	    dualSplit(list, lt+1, gt-1);
	    dualSplit(list, gt+1, maxIndex);
	}
